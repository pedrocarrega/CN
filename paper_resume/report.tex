...
A implementação do web server foi inicialmente realizada em NodeJS com a utilização de um router Koa. Porém a gramática das operações do Koa não estavam bem documentadas, pelo que mudamos a implementação para um router Express.

\subsection{Implementação - Base de dados}
...
\subsection{Lançamento em Kubernetes - deploy1.sh}
...
\subsection{Lançamento em Kubernetes - deploy2.sh}
...
\subsection{Lançamento em Kubernetes - deploy3.sh}
...
\subsection{Lançamento em Kubernetes - deploy4.sh}
