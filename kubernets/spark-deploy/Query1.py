...
cart = df 		.select(col("_c1").alias("event_type"), col("_c8").alias("sessions")) \
...
session = df \
		  ...
		  .groupby(col("session")) \
		  .agg(max(col("time")).alias("end_time"), min(col("time")).alias("start_time")) 		  .withColumn("duration", round(((col("end_time").cast("long") - col("start_time").cast("long"))/60), 1)) \
