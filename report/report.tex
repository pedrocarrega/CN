...
Nesta ultima parte do projeto foram implementados serviços Spark e a utilização de Terraform nos nossos \textit{scripts} de \textit{deployment}. Graças a conhecimentos previamente adquiridos, a implementação dos serviços Sparks foi um processo relativamente rápido. As complicações surgiram quando foi necessário integrar esses serviços com os restantes serviços já existentes devido a limitações impostas pelo serviço GCP. O Spark permitiu a implementação de serviços mais complexo quando comparado com os serviços fornecidos pela REST API. 
